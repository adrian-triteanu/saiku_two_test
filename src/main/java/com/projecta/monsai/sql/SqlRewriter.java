...
public class SqlRewriter {
    ...
    /**
     * Find constraints on a joined dimension table and add min/max restrictions
     * on the dimension table
     */
    private void addFactTableConstraints( Connection con ) {

        // iterate over all conditions
        for ( SqlFragment condition : fragments ) {
            if ( condition.getType() != FragmentType.CONDITION ) {
                continue;
            }
            ...
            // check if we can find a join condition on the same table, but a different column
            SqlFragment join = joins.get( tableAlias );
            if ( join == null || columnName.equals( join.getJoinedColumnName() ) ) {
                continue;
            }

            // check if the joined table is really a dimension table
            SqlFragment table = aliases.get( tableAlias );
            if ( table == null || !isDimensionTable( con, table ) ) {
                continue;
            }

            // retrieve the range restriction from the database
            Pair<Long, Long> range = getDimensionRange( con, table, join, condition );
            if ( range == null ) {
                continue;
            }

            condition.setNewText(
                    condition.getText() + "\n    "
                    + "and \"" + join.getTableAlias() + "\".\"" + join.getColumnName() + "\""
                    + " between " + range.getKey() + " and " + range.getValue() );
            modified = true;
        }
    }
    ...
    public static synchronized void clearCache() {
        ...
        dimensionTablesCache    = new HashMap<>();
        dimensionRangesCache    = new HashMap<>();
    }
    ...
    /**
     * Checks if the given table is not larger than MAX_DIM_TABLE_SIZE, so any
     * query on it will be effective
     */
    private static synchronized boolean isDimensionTable( Connection con, SqlFragment table ) {

        // check the cache
        String fullTableName = "\"" + table.getSchemaName() + "\".\"" + table.getTableName() + "\"";
        if ( dimensionTablesCache.containsKey( fullTableName ) ) {
            return dimensionTablesCache.get( fullTableName );
        }

        // query the table to check the size
        String query = "\nselect 1 from " + fullTableName
                     + " limit 1 offset " + MAX_DIM_TABLE_SIZE;
        ...
        try {
            try ( Statement stmt = con.createStatement();
                ...
                // we dont to expect any results if the table is short
                boolean isDimension = !rs.next();
                dimensionTablesCache.put( fullTableName, isDimension );
                return isDimension;
            }
        }
        catch ( Throwable e ) {
            LOG.error( "Error in SqlRewriter.isDimensionTable()", e );
            return false;
        }
    }
    ...
    /**
     * Retrieves the minimum and maximum range of primary keys in the dimension
     * table for the given condition
     */
    private static synchronized Pair<Long, Long> getDimensionRange( Connection con,
            SqlFragment table, SqlFragment join, SqlFragment condition ) {

        // generate sql query
        String query = "\nselect min(\"" + join.getJoinedColumnName() + "\") as min,"
                     + " max(\"" + join.getJoinedColumnName() + "\") as max"
                     + " from \"" + table.getSchemaName() + "\".\"" + table.getTableName() + "\""
                     + " as \"" + condition.getTableAlias() + "\""
                     + " where " + condition.getText().trim();

        // apply the cache timeout
        if ( System.currentTimeMillis() > dimensionRangesCacheTTL ) {
            dimensionRangesCache.clear();
            dimensionRangesCacheTTL = System.currentTimeMillis() + DIMENSION_RANGES_CACHE_TIMEOUT_MILLIS;
        }

        if ( dimensionRangesCache.containsKey( query )  ) {
            return dimensionRangesCache.get( query );
        }
        ...
        try {
            try ( Statement stmt = con.createStatement();
                ...
                if ( rs.next() ) {
                    ...
                    // we expect this to be a numeric column
                    int columnType = rs.getMetaData().getColumnType( 1 );
                    if ( columnType == Types.SMALLINT || columnType == Types.INTEGER || columnType == Types.BIGINT ) {
                ...
                dimensionRangesCache.put( query, range );
                return range;
            }
        }
        catch ( Throwable e ) {
            LOG.error( "Error in SqlRewriter.getDimensionRange()", e );
            return null;
        }
    }
